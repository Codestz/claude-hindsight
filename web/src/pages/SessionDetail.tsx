  ...
  useEffect(() => {
    ...
    es.onmessage = (evt) => {
      try {
        ...
        if (autoScrollRef.current && treePanelRef.current) {
          requestAnimationFrame(() => {
            treePanelRef.current?.scrollTo({ top: treePanelRef.current.scrollHeight, behavior: "smooth" });
          });
        }
      } catch {
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: active
                          ? "1px solid var(--accent)"
                          : "1px solid var(--border-2)",
                        background: active ? "rgba(0,255,136,0.08)" : "var(--bg-2)",
